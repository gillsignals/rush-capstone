...
```{r}
tokens %>%
  filter(track_title == "Cygnus X-1 Book I: The Voyage")
```

```{r}
tokens$word[tokens$word == "1six"] <- "six"
cygOne <- c("A Farewell To Kings", "Cygnus X-1 Book I: The Voyage", "1")
tokens <- rbind(tokens, cygOne)
```
...
Contractions are converted to their fully expanded form (e.g. "I'm" to "I am"). First, standard contractions in they key contraction table from the `lexicon` package were expanded, Then, nonstandard contractions and those missing from the key contraction table were expanded. Words ending with "`s" were trimmed to their roots for analysis with bag-of-words.

```{r}
tokens$word <- replace_contraction(tokens$word)

tokens$word <- mgsub(tokens$word, c("who've", "where'd", "wond'rin'why", "livin'yes", "trav'lin", "mem'ries", "ev'ry", "trav'llers", "o'er", "light'my"), c("who have", "where did", "wondering why", "living yes", "traveling", "memories", "every", "travelers", "over", "light my"))

```


```{r}
tokens$word <- str_remove(tokens$word, "'s")
```

The remaining words with apostrophes have the apostrophes as a key part of their spelling and do not need edits.
...
```{r}
gOmitted <- c("comin", "diggin", "feelin", "findin", "goin", "havin", "itchin", "laughin", "livin", "makin", "nothin", "rollin", "runnin", "smilin", "stoppin", "workin")

gReplaced <- c("coming", "digging", "feeling", "finding", "going", "having", "itching", "laughing", "living", "making", "nothing", "rolling", "running", "smiling", "stopping", "working")

tokens$word <- mgsub(tokens$word, gOmitted, gReplaced)
```
...
instrumentals <- data.frame(album = c("Hemispheres", "Moving Pictures", "Test Tor Echo"), track_title = c("La Villa Strangiato", "YYZ", "Limbo"), word = c(NA, NA, NA))

tokens <- rbind(tokens, instrumentals)
```

## Stop words

The Rush lyrics corpus contains 31,361 total words, of which 4,104 are unique. After removing stop words, there are 12,876 total words and 3701 unique words.
...
```{r}
save(tokens, file = "lyricTokens.RData")
```
